...
public class FanBlockBehavior extends BukkitBlockBehavior implements EntityBlockBehavior {
    ...
    public static class Factory implements BlockBehaviorFactory {
        ...
        public BlockBehavior create(CustomBlock block, Map<String, Object> args) {
            ...
            int tickDelay = Integer.parseInt(args.getOrDefault("tick-delay", 10).toString());
            Particle particle = Particle.valueOf(args.getOrDefault("particle", "CLOUD").toString().toUpperCase());
            int maxPushDistance = Integer.parseInt(args.getOrDefault("push-distance", 5).toString());
            int pushStrength = Integer.parseInt(args.getOrDefault("push-strength", 1).toString());

            double maxSpeedHorizontal = Double.parseDouble(args.getOrDefault("max-speed-horizontal", 0.8D).toString());
            double maxSpeedVertical  = Double.parseDouble(args.getOrDefault("max-speed-vertical",  0.6D).toString());
            boolean velocityCap = Boolean.parseBoolean(args.getOrDefault("enable-velocity-cap", true).toString());

            @SuppressWarnings("unchecked")
            List<String> passableBlocksList = (List<String>) args.getOrDefault("passable-blocks", List.of("minecraft:air"));
